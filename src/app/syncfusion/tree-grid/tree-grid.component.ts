...
export class TreeGridComponent implements OnInit {
    ...
    onSelected() {
        ...
        this.copiedRows.forEach((item: any) => {
            if (
                !selectedRows
                    .map((row) => this.getDataUid(row))
                    .includes(this.getDataUid(item))
            ) {
    ...
    getCopiedRecords() {
        ...
        return this.selectedRecords.filter(
            (item) => !parents.map((item) => item.taskID).includes(item.taskID)
        );
    }
    ...
    handlePaste() {
        ...
        const filterRecords: any = this.selectedRecords.filter(
            (record: any) =>
                !_.map(this.copiedRecords, 'taskID').includes(record.taskID)
        );
        ...
        if (this.isCut) {
            for (const record of this.copiedRecords) {
                ...
                _.set(
                    cloneTasks,
                    `${path}.subtasks`,
                    _.get(cloneTasks, `${path}.subtasks`).filter(
                        (item) => item.taskID !== record.taskID
                    )
                );
            }
